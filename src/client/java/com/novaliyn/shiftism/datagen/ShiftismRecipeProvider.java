	...
	protected RecipeProvider createRecipeProvider(HolderLookup.Provider registryLookup, RecipeOutput exporter) {
		return new RecipeProvider(registryLookup, exporter) {
			...
			public void buildRecipes() {
				...
				shaped(RecipeCategory.TOOLS, ShiftedItems.shifted_axe, 1).pattern(" ii").pattern(" gi").pattern(" s ").define('g', ShiftedItems.shifted_gem).define('i', ShiftedItems.shifted_ingot).define('s', ShiftedItems.shifted_rod).unlockedBy(getHasName(ShiftedItems.shifted_gem), (has(ShiftedItems.shifted_gem))).save(output);
				shaped(RecipeCategory.TOOLS, ShiftedItems.shifted_shovel, 1).pattern(" g ").pattern(" s ").pattern(" s ").define('g', ShiftedItems.shifted_gem).define('s', ShiftedItems.shifted_rod).unlockedBy(getHasName(ShiftedItems.shifted_gem), (has(ShiftedItems.shifted_gem))).save(output);
				shaped(RecipeCategory.TOOLS, ShiftedItems.shifted_hoe, 1).pattern("ii ").pattern(" g ").pattern(" s ").define('g', ShiftedItems.shifted_gem).define('i', ShiftedItems.shifted_ingot).define('s', ShiftedItems.shifted_rod).unlockedBy(getHasName(ShiftedItems.shifted_gem), (has(ShiftedItems.shifted_gem))).save(output);
